...
parser.add('--remove_blow_sampling', type=int, default=0, help = 'remove blow sampling transformation')
...
if args.remove_blow_sampling == 1:
...
def append_experiment_info(df, model_path, feat_option, method, use_aux, metric, qc_split = None, pca = None):
...
if method == 'RidgeRegression':
    ...
    model = linear_model.LogisticRegressionCV(penalty = 'l2', solver = 'liblinear', random_state=0, n_jobs=-1, max_iter = int(1e4))
...
path = './Files/Results/SimCLR_Probe_bootstrap'
downstream_model_path = './Files/models'

if args.save_downstream_model == 1:
    filename = downstream_model_path + '/explain_%s_%s.pkl' % (method, args.val_idx_list)
    save_dict(model, path = filename)
...
for idx_list, save_path in zip(subset_list, save_path_list):
    ...
    # output_aux_accy, output_aux_auc, output_aux_counter = evaluate_LP_aux(pred, scores, t_feature_labels, t_aux_bank, aux_col)
    ...
    for tmp_df, metric in zip(output, metrics):
        tmp_df = append_experiment_info(tmp_df, model_path, feat_option, method, use_aux, metric, pca = args.pca)
